    ...
    if (source.id !== target.id && source.layer < target.layer &&/*FUCK UP*/ check1 === -1 && check2 === -1 /*FUCK UP*/) {
        ...
        this.activate = (charge) => {
            if (this.active == true) {
                ...
                brain.activations++;
                //that.updateBias(charge);
                //console.log('Connection ' + this.id + ' now has a memory of [' + this.recentActivations.toString() + '] resulting in a bias of ' + this.bias);
                //console.log('Connection ' + this.id + ' transmitting charge of: ' + this.bias);
                if (this.target.active == true) {
                    //that.target.transmit(this.bias);
